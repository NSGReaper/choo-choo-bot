...
module.exports.registerGame = async (gameId, initialGameData) => {
    ...
    if (!game.exists()) {
        ...
        game.put(Object.assign({}, initialGameData, data))
    } else {
        if (Array.isArray(game.gameData.subscriptions) && Array.isArray(initialGameData.subscriptions)) {
            const subscriptions = game.gameData.subscriptions.concat(initialGameData.subscriptions)
            game.put(Object.assign(initialGameData, subscriptions))
        } else {
